<template>
     <div class="page">
        <h4 style="text-align:center;font-size:.1rem;">属性区</h4>
        <div v-if="!formType">
        ...
        <!--轮播图属性-->
            <div v-if="formType === 'carousel'">
                <el-card class="box-card" v-for="(item, index) in carouselArray" :key="index">
                    <el-form label-width=".4rem" class="carousel-item">
                    ...
                    <el-form label-width=".4rem">
                        ...
                        <div class="bigdiv" style='position:relative;'>
                          ...
                          <el-form-item style="margin-top:20px;width:100%;" label="编辑授权">
                              ...
                              <template>
                                <div style="width:100%;text-align:center;border:1px solid #ccc;border-radius:3px;height:.85rem;margin-top:.1rem;overflow-y:scroll;">
                              ...
                              <div v-show="tableShow1" style="position: absolute;top: .325rem;left: 1%;width: 97%">
                                <el-table
                                ref="singleTable"
                                :data="tableData1"
                                max-height="150"
                        ...
                        <!-- 限制查看开关按钮 -->
                        <template>
                          <div style="width:100%;height:.2rem;text-align: left;line-height:.2rem;padding-left: 10px">
                            <el-switch
                              v-model="modelCheck"
                              @change="onChange"
                              inactive-text="限制查看">
                        ...
                        <el-form-item v-show="linkCheck" class="btnContent" style="margin-top:.1rem;margin-left:0;width:100%;" >
                          <template>
                            <div style="width:100%;border:1px solid #ccc;border-radius:3px;float:right;overflow-y:scroll;">
                                <div class="btngroup" :key="index" v-for="(item,index) in btnArr">                                
                                  <div style="width:13%;float:left;border-radius:4px;border: 1px solid #dcdfe6;">
                        ...
                        <el-form-item style="margin-top:.1rem;width:100%;" label="类型" >
                        ...
                        <!--内容-->
                        <template>
                            <el-button style="width:44%;height:.2rem;margin-left:28%;" v-show="manageMent.producttype == '内容'" type="primary" size="small" @click="editFileClick">编辑</el-button>
                        </template>
                    </el-form>
                </el-card>
            </div>
            ...
            <div v-if="formType === 'btnLink'">
                ...
                <el-form>
                    <el-form-item label="关于我们" style="text-align:center;">
                         <el-button style="width:40%;margin-left:17%;" type="primary" size="small" @click= 'exitClcik'>编辑</el-button>
                    </el-form-item>
                </el-form>
            </div>

            <div v-if="formType" style="width:100%;height:.2rem;margin-top:20px;padding-bottom:.5rem;">
                <el-button style="width:40%;margin-left:30%;height:.2rem;" type="primary" @click="saveClick">保存</el-button>
            </div>
     </div>
</template>
    ...
    manageMent() {
      ...
      if (!isAdmiId.productprop) {
        this.$store.state.attribute.managementArr[
          ...
          this.$store.state.attribute.chooseLableIndex
        ].productprop = {};
        // 构造指定数据结构
        this.$store.state.attribute.managementArr[
          this.$store.state.attribute.choosManIndex
      ...
      if (productpropType == "string") {
        this.allPeopleArr = JSON.parse(
          this.$store.state.attribute.managementArr[
            this.$store.state.attribute.choosManIndex
          ].DataVal[this.$store.state.attribute.chooseLableIndex].productprop
        ).allPeopleArr;

        this.linkUrl = JSON.parse(
          this.$store.state.attribute.managementArr[
            this.$store.state.attribute.choosManIndex
          ].DataVal[this.$store.state.attribute.chooseLableIndex].productprop
        ).linkUrl;
        ...
        if (
          ...
          this.btnArr = this.$store.state.attribute.managementArr[
            ...
            this.$store.state.attribute.chooseLableIndex
          ].productprop.btnLinkArr;
          if (
            this.btnArr[this.btnArr.length - 1].btnName &&
            this.btnArr.length < 4
          ) {
            let obj = {};
            obj.btnImgurl = "";
            obj.btnName = "";
